...
func _check_input_numpad(numpad_input_string: String, input_window: int, buffer_size: int) -> bool:
	...
	var counter = numpad_input_string.length() - 1
	var total_frames = 0
	
	for input in recent:
		# if input is detected in-order, advance counter
		if counter < 0 :
			match numpad_input_string[counter]:
				"1":
					# down + back
				...
				"9":
					...
					if input.get_dir() == Vector2(1,1):
						counter -= 1
				"U":
					# any upwards direction
					if input.get_dir().y > 0:
						counter -= 1
				"D":
					# any downwards direction
					if input.get_dir().y < 0:
						counter -= 1
				"F":
					# any forwards direction
					if input.get_dir().x > 0:
						counter -= 1
				"B":
					# any backwards direction
					if input.get_dir().x < 0:
						counter -= 1
		
		# add total frames until least recent input
		# (does not count least recent)
		if counter != 0:
			total_frames += input.get_held_frames()
	
	if counter == 0 and total_frames <= input_window:
		return true
	else:
		return false

func _check_charge_input(charge_input_string: String, release_input_string: String, charge_time) -> bool:
	var recent = buffer.slice(0, 4)
